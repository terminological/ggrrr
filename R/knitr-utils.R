        ...
        paste0(
          ...
          paste0(
            lapply(1:length(starts), function(i) {
              paste0(
                lines[
                  min(starts[[i]], length(lines)):min(ends[[i]], length(lines))
                ],
                collapse = "\n"
              )
            }),
...
code_snip = function(
  ...
  if (!fs::file_exists(filename)) {
    ...
    if (length(starts) != length(ends)) {
      stop("Mismatch in start and end markers, start:{}, end:{}", starts, ends)
    }
