...
#'
#' }
check_font = function(family, .legacy = FALSE) {
  if (.Platform$OS.type == "windows") {
    # No easy way to use webfonts in windows, because it .
    ...
    if (.legacy) {
      .import_systemfont_to_grdevices(sub)
      sub[!sub %in% names(grDevices::pdfFonts())] = NA
    }
    ...
    if (.legacy) {
      .import_systemfont_to_grdevices(family)
    }
    ...
    if (length(switched) > 0) {
      ...
      if (.legacy) .import_systemfont_to_grdevices(switched)
...
N.b. This will NOT remove any custom fonts installed on your system by `ggrrr`."
  ...
  if (confirm) {
    ...
    if (fonts) {
      try(fs::dir_delete(.font_cache_loc()), silent = TRUE)
    }

    # try(
    #   fs::dir_delete(fs::path(system.file(package = "extrafontdb"), "metrics")),
    #   silent = TRUE
    # )
    # try(
    #   fs::file_delete(fs::path(
...
rebuild_fonts = function() {
  ...
  .import_systemfont_to_grdevices()
  invisible(NULL)
}

## File locations ----

.font_cache_loc = function(filename = "") {
  fs::dir_create(fs::path(
    fs::path_expand_r(rappdirs::user_cache_dir("ggrrr")),
    "fonts"
  ))
  fs::path(
    fs::path_expand_r(rappdirs::user_cache_dir("ggrrr")),
...
.search_webfont_services = function(
  fonts,
  services = names(webfont_provider),
  ...
) {
  font_family = NULL
...
.get_webfont_catalogue = function() {
  if (!exists("x", envir = .webfont_catalogue, inherits = FALSE)) {
    if (fs::file_exists(.font_cache_loc("webfonts.csv"))) {
      tmp = readr::read_csv(
        .font_cache_loc("webfonts.csv"),
...
.add_webfont_catalogue = function(css_df) {
  ...
  readr::write_csv(
    tmp,
    file = .font_cache_loc("webfonts.csv")
  )
}

.reset_webfont_catalogue = function() {
  try(fs::file_delete(.font_cache_loc("webfonts.csv")), silent = TRUE)
  assign("x", .empty_webfont_catalogue(), envir = .webfont_catalogue)
}
...
.embed_fonts = function(pdfFile, fonts) {
  ...
  paths = dplyr::bind_rows(
    ...
    dplyr::filter(family %in% fonts) %>%
    dplyr::pull(path)

  suppressMessages(
    grDevices::embedFonts(file = fs::path_expand(pdfFile), fontpaths = paths)
  )
}
...
.style_to_css_weight = function(style) {
  dplyr::case_when(
    ...
    stringr::str_detect(tolower(style), stringr::fixed("extralight")) ~ "200",
    ...
    stringr::str_detect(tolower(style), stringr::fixed("semibold")) ~ "600",
    stringr::str_detect(tolower(style), stringr::fixed("extra bold")) ~ "800",
    stringr::str_detect(tolower(style), stringr::fixed("extrabold")) ~ "800",
...
webfont_provider = list(
  ...
  google = function(fonts) {
    sprintf(
      ...
      paste0(
        sprintf(
          "family=%s:ital,wght@0,400;0,700;1,400;1,700",
          .esc(fonts)
        ),
        collapse = "&"
      )
...
.parse_css = function(request_url) {
  ...
  tmp = tibble::tibble(
    font_family = stringr::str_extract(
      info,
      "font-family:[[:space:]]*([^:;]+);",
      group = 1
    ) %>%
      .unquote(),
    font_style = stringr::str_extract(
      info,
      "font-style:[[:space:]]*([^:;]+);",
      group = 1
    ),
    font_weight = stringr::str_extract(
      info,
      "font-weight:[[:space:]]*([[:digit:]]+);",
      group = 1
    ),
    url = stringr::str_extract(info, "url\\(([^\\)]+)\\)", group = 1) %>%
      stringr::str_replace("^//", "https://"),
    format = stringr::str_extract(info, "format\\(([^\\)]+)\\)", group = 1) %>%
      .unquote(),
    local = stringr::str_extract(info, "local\\(([^\\)]+)\\).*", group = 1) %>%
      .unquote(),
    unicode_range = stringr::str_extract(
      info,
      "unicode-range:[[:space:]]*([^:;]+);",
      group = 1
    )
...
.get_font_face = function(fonts) {
  ...
  tmp = .get_webfont_catalogue() %>%
    dplyr::filter(font_family %in% fonts) %>%
    dplyr::mutate(
      ps_face = .style_to_ps_face(
        css_weight = font_weight,
        css_style = font_style
      )
    )
  tmp2 = .all_system_fonts() %>%
    dplyr::filter(family %in% fonts & !is.na(ps_face)) %>%
    dplyr::select(family, ps_face, source) %>%
    dplyr::distinct()
  tmp3 = tmp %>%
    dplyr::left_join(tmp2, by = c("font_family" = "family", "ps_face"))
  tmp4 = tmp3 %>%
    dplyr::mutate(
  ...
  tmp5 = tmp4 %>%
    ...
    dplyr::mutate(font_face = purrr::pmap(., svglite::font_face))
  tmp6 = tmp4 %>%
    dplyr::filter(is.na(local)) %>%
    dplyr::select(-local) %>%
    dplyr::mutate(font_face = purrr::pmap(., svglite::font_face))
...
#'
.find_and_download_web_fonts = function(
  fonts,
  services = names(webfont_provider),
  ...
) {
  ...
  missing_webfonts = webfonts %>%
    dplyr::filter(!.check_systemfonts(font_family))

  # download any new fonts.
  tmp = missing_webfonts %>%
    dplyr::mutate(
      face = .style_to_ps_face(
        css_weight = font_weight,
        css_style = font_style
      ),
      extn = .css_format_to_extn(format),
      ...
      local = ifelse(
        face == "plain",
        .font_loc(sprintf("%s.%s", font_family, extn)),
        NA_character_
      ),
    ) %>%
    dplyr::mutate(
      ttf = purrr::map2_chr(
        ...
        locn,
        ~ .get_web_ttf(.x, .y),
        .progress = "Downloading fonts"
      )
    )

  tmp %>%
    dplyr::filter(!is.na(local)) %>%
    purrr::pwalk(.f = function(locn, local, ...) {
      if (!fs::file_exists(local)) fs::file_copy(locn, local)
    })
...
.register_web_fonts_with_systemfont = function(
  webfonts = .get_webfont_catalogue()
) {
  ...
  unregistered_with_systemfonts = webfonts %>%
    ...
    dplyr::anti_join(
      .all_system_fonts(valid_ps = FALSE),
      by = c("font_family" = "family", "ps_face")
    )

  # register any newly downloaded fonts with systemfonts
  tmp2 = unregistered_with_systemfonts %>%
    dplyr::mutate(
      face = .style_to_ps_face(
        css_weight = font_weight,
        css_style = font_style
      ...
      extn = .css_format_to_extn(format),
      ttf = .font_cache_loc(sprintf("%s-%s.%s", font_family, face, extn))
    ) %>%
    dplyr::select(
      name = font_family,
      face,
  ...
  outcome = tmp2 %>%
    purrr::pwalk(
      purrr::safely(systemfonts::register_font),
      .progress = "Registering fonts with systemfonts"
    )

  matched = webfonts %>%
    dplyr::left_join(
      .all_system_fonts(valid_ps = FALSE),
      by = c("font_family" = "family")
    ) %>%
...
# # Deprecated extrafont removed from CRAN
#
# #' @noRd
# #' @examples
# #' .import_systemfont_to_extrafont("Courier New")
# #' .import_systemfont_to_extrafont("Kings")
# .import_systemfont_to_extrafont = function(
#   family = .all_system_fonts(ttf_only = TRUE)$family
# ) {
...
#     localfonts = unique(
#       .all_system_fonts(ttf_only = TRUE) %>%
#         dplyr::filter(source == "system") %>%
#         dplyr::pull(family)
#     )
#   } else {
#     localfonts = unique(
#       .all_system_fonts(ttf_only = TRUE) %>% dplyr::pull(family)
#     )
#   }
...
#   tmp %>%
#     purrr::pwalk(
#       purrr::safely(.extrafont_ttf_import),
#       .progress = "Importing to extrafonts"
#     )
#
...
.all_system_fonts = function(valid_ps = FALSE, ttf_only = valid_ps) {
  ...
  tmp = dplyr::bind_rows(
    systemfonts::registry_fonts() %>%
      dplyr::mutate(weight = as.character(weight), source = "registry"),
    systemfonts::system_fonts() %>%
      dplyr::mutate(weight = as.character(weight), source = "system")
  ) %>%
    dplyr::mutate(
  ...
  if (ttf_only) {
    tmp = tmp %>% dplyr::filter(!is.na(ps_face) & type == "ttf")
  }
  if (valid_ps) {
    tmp = tmp %>%
      dplyr::group_by(family, monospace, ps_face) %>%
...
.substitute_fonts = function(family, quiet = TRUE) {
  ...
  sys_fonts_list = bind_rows_with_factors(
    systemfonts::registry_fonts(),
    systemfonts::system_fonts()
  ) %>%
...
#   afm_files = .font_cache_loc(
#     fs::path_file(ttf_files) %>% fs::path_ext_set("afm")
#   )
#   afmgz_files = fs::path(
#     metrics_path,
#     fs::path_file(ttf_files) %>% fs::path_ext_set("afm.gz")
#   )
#   # errors ignored:
#   purrr::map2(
#     ttf_files,
#     afm_files,
#     purrr::possibly(~ .exec_ttf2afm(.x, .y, family = family))
#   )
#   afm_compressed = purrr::map2(
#     afm_files,
#     afmgz_files,
#     purrr::possibly(~ .gz(.x, .y))
#   ) %>%
#     purrr::list_c()
#   # errors excluded:
...
#   fontdata = fontdata %>%
#     dplyr::mutate(
#       Bold = stringr::str_detect(weight, "Bold") |
#         stringr::str_detect(fontname, "Bold|12"),
#       Italic = stringr::str_detect(weight, "Italic|Oblique") |
#         stringr::str_detect(fontname, "Italic|Oblique"),
...
#
#   if (fs::file_exists(fmap)) {
#     tmp2 = readr::read_lines(fmap, progress = FALSE)
#   } else {
#     tmp2 = character()
#   }
#   tmp2 = sort(unique(c(
#     tmp2,
#     sprintf("/%s (%s) ;", fontdata$FontName, fontdata$fontfile)
#   )))
...
#' @noRd
.import_systemfont_to_grdevices = function(
  family = .all_system_fonts(valid_ps = TRUE)$family
) {
  localfonts = unique(.all_system_fonts()$family)
  # check what is locally available in systemfonts but not registered with grDevices
  toadd = .grdevices_missing(dplyr::intersect(family, localfonts))
  if (!"bold" %in% colnames(toadd)) {
    toadd$bold = toadd$plain
  }
  if (!"italic" %in% colnames(toadd)) {
    toadd$italic = toadd$plain
  }
  if (!"bolditalic" %in% colnames(toadd)) {
    toadd$bolditalic = toadd$plain
  }
...
.grdevices_missing = function(family) {
  ...
  both_present = family[
    .check_postscript_font(family) & .check_pdf_font(family)
  ]
  ...
  installable = installable %>%
    dplyr::group_by(name) %>%
    tidyr::complete(ps_face = c("plain", "bold", "italic", "bolditalic")) %>%
    #dplyr::mutate(path = ifelse(is.na(path), na.omit(path)[1], path)) %>%
    #dplyr::glimpse()
    tidyr::fill(path, .direction = "downup") %>%
    dplyr::group_by(name, ps_face) %>%
    dplyr::filter(dplyr::row_number() == 1) %>%
    dplyr::group_by(name) %>%
...
.register_ttf = function(
  ...
  tmp = tmp %>%
    dplyr::group_by(name) %>%
    dplyr::mutate(
      loc = .font_cache_loc(sprintf("%s-%s.afm", name, face)),
      afm = dplyr::if_else(
        ...
        purrr::map2_chr(
          ...
          loc,
          purrr::possibly(
            ~ .exec_ttf2afm(.x, .y, family = unique(name)),
            otherwise = NA_character_
          )
          # .progress = "Converting TTF to AFM"
        ),
        NA_character_
      )
    ) %>%
  ...
  tmp2 = tmp %>%
    dplyr::filter(!registered_ps & !missing_afm) %>%
  ...
  tmp3 = tmp %>%
    dplyr::filter(!registered_pdf & !missing_afm) %>%
    dplyr::select(name, type1)
  arg3 = tmp3$type1
  ...
  binary = .ttf2afm_binary()

  tmp = tmp %>%
    dplyr::mutate(
      status = dplyr::case_when(
        is.null(binary) ~ "SKIP: no binary for ttf2afm",
        registered_pdf & registered_ps ~
          sprintf("SKIP: %s already registered", name),
        missing_afm ~ sprintf("SKIP: conversion failed for the font %s", name),
        new_registered_pdf & new_registered_ps ~
          sprintf("OK: registered %s for ps and pdf", name),
...
.ttf2afm_call = function(ttf, afm) {
  ...
  #         "fAe",
  #         shQuote(ttf),
  #         shQuote(fs::path_ext_remove(afm))
  #       )
  #     } else {
  #       args = c("-a", "-GfAe", shQuote(ttf), shQuote(fs::path_ext_remove(afm)))
  #     }
  #
  ...
  # request unicode glyph names
  args = c("-u", "-o", shQuote(afm), shQuote(ttf))

  res = suppressMessages(suppressWarnings(system2(
    enc2native(binary),
    args,
    stdout = NULL,
    stderr = TRUE,
  ...
  if (isTRUE(attr(res, "status") != 0)) {
    stop(e$message, "\nttf2afm error: ", res)
  }
  #  })
  if (!fs::file_exists(afm)) {
    stop("afm conversion failed, unknown reason.")
  }
  return(afm)
}
...
#' converted_afm = .ttf2afm_call(ttf, afm)
#' tmp = readr::read_lines(converted_afm)
#'
#' try(.exec_ttf2afm(tempfile(), tempfile(), overwrite=TRUE))
.exec_ttf2afm = function(
  ttf,
  afm = fs::path_ext_set(ttf, "afm"),
  overwrite = FALSE,
  family = NULL
) {
  # cache
  if (fs::file_exists(afm) && !overwrite) {
    return(afm)
  }
...
.get_afm_meta = function(afm) {
  ...
  tibble::tibble(
    fontname = stringr::str_remove(
      tmp[stringr::str_starts(tmp, "FontName ")],
      "FontName "
    ),
    fullname = stringr::str_remove(
      tmp[stringr::str_starts(tmp, "FullName ")],
      "FullName "
    ),
    familyname = stringr::str_remove(
      tmp[stringr::str_starts(tmp, "FamilyName ")],
      "FamilyName "
    ),
    weight = stringr::str_remove(
      tmp[stringr::str_starts(tmp, "Weight ")],
      "Weight "
    ),
    italicangle = stringr::str_remove(
      tmp[stringr::str_starts(tmp, "ItalicAngle ")],
      "ItalicAngle "
    )
  )
}
...
.set_afm_meta = function(
  ...
  if (!is.null(fontname)) {
    tmp[min(which(stringr::str_starts(tmp, "FontName ")))] = sprintf(
      "FontName %s",
      fontname
    )
  }
  if (!is.null(fullname)) {
    tmp[min(which(stringr::str_starts(tmp, "FullName ")))] = sprintf(
      "FullName %s",
      fullname
    )
  }
  if (!is.null(familyname)) {
    tmp[min(which(stringr::str_starts(tmp, "FamilyName ")))] = sprintf(
      "FamilyName %s",
      familyname
    )
  }
  if (!is.null(weight)) {
    tmp[min(which(stringr::str_starts(tmp, "Weight ")))] = sprintf(
      "Weight %s",
      weight
    )
  }
  if (!is.null(italicangle)) {
    tmp[min(which(stringr::str_starts(tmp, "ItalicAngle ")))] = sprintf(
      "ItalicAngle %s",
      italicangle
    )
  }

  head = tmp[!grepl("^(C |End)", tmp)]
  tail = tmp[grepl("^End", tmp)]
  metrics = tmp[grepl("^C ", tmp)]

  # Should be empty unless there are agl mappings in the ttf... just in case
  mapped_agl = metrics[
    !grepl("^.*(uni[0-9A-F]{4}|index[0-9]{1-4}).*$", metrics)
  ]
  # The ones where unicode mappings were supplied in the ttf:
  mapped_unicode = metrics[grepl("^.*(uni[0-9A-F]{4}).*$", metrics)]
  # Nothing we can do about these ones:
  not_mapped = metrics[grepl("^.*(index[0-9]{1-4}).*$", metrics)]

  unicode_df = data.frame(
    line = mapped_unicode,
    unicode = gsub("^.*uni([0-9A-F]{4}).*$", "\\1", mapped_unicode)
  )

  # map accross to glyph names using an inner join. This can create duplicates.
  merged = merge(unicode_df, ggrrr::glyphmap)

  # rebuild metric line:
  merged$newline =
    sapply(seq_along(merged$line), function(i) {
      # Swap unicode for agl in line
      gsub(
        "(^.*)uni[0-9A-F]{4}(.*$)",
        sprintf("\\1%s\\2", merged$agl[i]),
        merged$line[i]
      )
    })

  # We need to dedupe the mapped ones - we just take the first row:
  # This should be orde
  dedup = by(merged, INDICES = factor(merged$agl), FUN = function(grp) grp[1, ])
  dedup_df = array2DF(dedup, simplify = TRUE)

  rep = c(head, mapped_agl, dedup_df$newline, tail)

  # # the AFM haas unicode names rather than glyph names. (because of the -u flag)
  # # extract the unicode
  # uni = gsub("^.*uni([0-9A-F]{4}).*$", "\\1", tmp)
  #
  # # match unicode to first xmap to glyph or NA
  # firstmatch = sapply(
  #   uni,
  #   function(u) {
  #     x = which(u == glyphmap$unicode)
  #     if (length(x) == 0) NA else x[[1]]
  #   },
  #   USE.NAMES = FALSE
  # )
  #
  # # pull the glyphs for the unicode
  # glyph = glyphmap$agl[firstmatch]
  #
  # # replace unicode for glyph code
  # rep = sapply(seq_along(tmp), function(i) {
  #   gsub("^(.*)uni[0-9A-F]{4}(.*)$", sprintf("\\1%s\\2", glyph[i]), tmp[i])
  # })
  #
  # # get rid of lines where glyphcode not found (like this so that unmatching lines
  # # at start are preserved)
  # rep = rep[rep == tmp | !is.na(glyph)]

  readr::write_lines(rep, file = afm, append = FALSE)
  return(afm)
}
#
# #' @noRd
...
#     if (
#       fs::file_info(infile)$modification_time <=
#         fs::file_info(outfile)$modification_time
#     ) {
#       return(outfile)
#     }
#   }
