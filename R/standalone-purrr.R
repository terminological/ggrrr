...
#
# 2025-11-17
# * Progress bar support using cli
# * qualified rlang namespace
# *
#
# 2023-02-23:
# * Added `list_c()`
...
.wrap_with_progress = function(
  ...
  if (!isFALSE(.progress)) {
    id = cli::cli_progress_bar(
      auto_terminate = TRUE,
      name = if (is.character(.progress)) .progress else NULL,
      total = .total,
      type = "iterator",
      .envir = .user_env
    )
    .g = function(...) {
      tmp = .f(...)
      cli::cli_progress_update(id = id, .envir = .user_env)
      return(tmp)
    }
    return(.g)
  } else {
