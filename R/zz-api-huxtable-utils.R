...
fit_col_widths = function(table) {
  ...
  table %>%
    ...
    dplyr::mutate(
      ar = .get_text_ar(label, font = fontName, face = fontFace) %>%
        dplyr::pull(ar)
    ) %>%
...
.get_text_ar = function(txt, font, face = "plain", linespacing = 1 + 1 / 8) {
  ...
  tmp = tmp %>%
    dplyr::mutate(
      ...
      ar = purrr::pmap_dbl(
        list(l = label, f = font, ff = face, ll = lines),
        function(l, f, ff, ll) {
          tryCatch(
            {
              grob = suppressWarnings(grid::textGrob(
                l,
                gp = grid::gpar(fontfamily = f, fontface = ff, fontsize = 8)
              ))
            ...
            error = function(e) {
              # browser()
              return(
                stringr::str_length(l) / (ll + (ll - 1) * (linespacing - 1))
              )
            }
          )
        }
...
.get_text_cms <- function(
  ...
  ret = ret %>%
    dplyr::mutate(
      heightCms = (lines + (lines - 1) * (linespacing - 1)) *
        font_size /
        72 *
        2.54,
...
hux_auto_widths = function(hux, target = "html", including_headers = FALSE) {
  value = strwidth = minStrwidth = newwidth = NULL # remove global binding note
  ...
  strwidths = tmp %>%
    ...
    dplyr::summarise(dplyr::across(
      tidyr::everything(),
      .fns = ~ max(.get_text_cms(.x, font_size = fontSize), na.rm = TRUE)
    )) %>%
    tidyr::pivot_longer(cols = tidyr::everything()) %>%
    dplyr::rename(strwidth = value)
  ...
  words = tmp %>%
    ...
    dplyr::summarise(dplyr::across(
      tidyr::everything(),
      .fns = ~ stringr::str_split(stats::na.omit(.x), stringr::fixed(" ")) %>%
        lapply(length) %>%
        unlist() %>%
        max()
    )) %>%
    tidyr::pivot_longer(cols = tidyr::everything()) %>%
    dplyr::rename(words = value)
  ...
  wordLength = tmp %>%
    ...
    dplyr::summarise(dplyr::across(
      tidyr::everything(),
      .fns = ~ stringr::str_split(stats::na.omit(.x), stringr::fixed(" ")) %>%
        lapply(function(.x) .get_text_cms(.x, font_size = fontSize)) %>%
        unlist() %>%
        max()
    )) %>%
    tidyr::pivot_longer(cols = tidyr::everything()) %>%
    dplyr::rename(minStrwidth = value)
  # browser()
  charLen = strwidths %>%
    dplyr::inner_join(rowspans, by = "name") %>%
    dplyr::inner_join(words, by = "name") %>%
    dplyr::inner_join(wordLength, by = "name") %>%
    dplyr::mutate(
      newwidth = pmax(
        minStrwidth,
        strwidth *
