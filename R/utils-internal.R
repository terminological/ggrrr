...
.optional_fn = function(
  ...
  if (
    rlang::is_missing(name) ||
      rlang::is_function(name) ||
      rlang::is_formula(name)
  ) {
    if (rlang::is_function(name) || rlang::is_formula(name)) {
      alt = name
    }
    tmp = unlist(stringr::str_split(pkg, stringr::fixed("::")))
    if (length(tmp) != 2) {
      stop(
        "either `name` must be supplied or the format `package::function` used for the `pkg` parameter"
      )
    }
    pkg = tmp[1]
    name = tmp[2]
